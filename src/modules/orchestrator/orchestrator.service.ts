import { deriveTraderMetrics } from "../scoring/metrics";
import { checkEligibility } from "../scoring/eligibility";
import { calculateScore } from "../scoring/engine";
import { assignTierFromScore } from "../scoring/tiers";
...
import { validateTraderData } from "../scoring/validation";

const DEFAULT_MAX_ATTEMPTS_PER_TRADER = 1;
const DEFAULT_BATCH_SIZE = 100;

export class OrchestratorService {
  constructor(private readonly snapshotService: SnapshotService = new SnapshotService()) {}
  ...
  async runPipeline(
    ...
    const batches = splitIntoBatches(input.traders, batchSize);

    const processed: ProcessedTrader[] = [];
      ...
      const results = await Promise.all(
        batch.map((trader) =>
          withRetry(
            ...
            maxAttempts
          ).catch((error) => this.toFailedResult(trader, error))
        )
      );
  ...
  private async processTrader(trader: PipelineTraderInput): Promise<ProcessedTrader> {
    ...
    const metrics = deriveTraderMetrics(trader.trades);

    // Validate trader + trades + metrics
    const validationResult = validateTraderData(
      {
        id: trader.traderId,
        status: "ACTIVE",
        metadata: {},
    ...
    // Tier resolver
    const tierResult = assignTierFromScore(score);
  ...
  private applyRanking(traders: ProcessedTrader[]): ProcessedTrader[] {
  ...
  private async persistSnapshotIfAny(
    input: PipelineInput,
    traders: ProcessedTrader[]
  ): Promise<string | null> {
    ...
    const snapshotTraders: TraderSnapshotInput[] = successful.map((t) => ({
      traderId: t.traderId,
      externalTraderId: t.externalTraderId,
      score: t.score ?? 0,
      rank: t.rank ?? 0,
      tier: (t.tier ?? TierLevel.BRONZE) as unknown as TraderTier,
      metrics: {
  ...
  private buildSummary(input: PipelineInput, traders: ProcessedTrader[]): PipelineSummary {
